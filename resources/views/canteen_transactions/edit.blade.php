<x-app-layout>
    <div class="mb-6 flex items-center">
        ...
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Ubah Transaksi Kantin</h2>
            <p class="text-gray-500 text-sm mt-1">Ubah catatan aktivitas tap kantin.</p>
        </div>
    </div>
    ...
    <div class="bg-blue-50 border border-blue-200 text-blue-700 p-4 mb-6 rounded-2xl flex items-center max-w-4xl">
        ...
        <div>
            ...
            <p class="text-sm">Satu kali Tap dihargai sebesar <strong>Rp 12.000</strong>. Sistem akan otomatis menghitung ulang totalnya ketika Anda mengubah jumlah Tap.</p>
        </div>
    </div>

    <div class="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 max-w-4xl">
        <form action="{{ route('canteen-transactions.update', $canteenTransaction->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    <select name="employee_id" id="employee_id" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-gray-800 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 focus:bg-white transition-colors" required>
                        ...
                        @foreach($employees as $emp)
                            <option value="{{ $emp->id }}" {{ old('employee_id', $canteenTransaction->employee_id) == $emp->id ? 'selected' : '' }}>
                                {{ $emp->nik }} - {{ $emp->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="date" name="transaction_date" id="transaction_date" value="{{ old('transaction_date', $canteenTransaction->transaction_date) }}" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-gray-800 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 focus:bg-white transition-colors" required>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="number" min="1" step="1" name="tap_count" id="tap_count" value="{{ old('tap_count', $canteenTransaction->tap_count) }}" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-gray-800 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 focus:bg-white transition-colors" required>
