<x-app-layout>
    <div class="mb-6 flex items-center">
        ...
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Ubah Data Absensi (Manual)</h2>
            <p class="text-gray-500 text-sm mt-1">Mengubah data kehadiran yang tidak sesuai atau error.</p>
        </div>
    </div>
        ...
        <form action="{{ route('attendances.update', $attendance->id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="date" name="date" id="date" value="{{ old('date', $attendance->date) }}" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-gray-800 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 focus:bg-white transition-colors" required>
                ...
                <div class="md:col-span-2 pt-4">
                    <label class="flex items-center cursor-pointer">
                        <input type="checkbox" name="is_manual" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500 w-5 h-5 mr-3" {{ $attendance->is_manual ? 'checked' : '' }}>
                        <span class="text-sm font-medium text-gray-700 hover:text-indigo-600 transition-colors">Tanda Perubahan Manual. Centang ini jika mengubah data karena sistem error.</span>
                    </label>
                </div>
            </div>
